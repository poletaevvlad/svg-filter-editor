...
class CodeDisplay extends React.Component{
	...
	makeLine(lines, tag, offset){
		...
		for (let arg in tag.args){
			args.push(<span key={arg}> <span className="attr">{this.getAttribute(arg)}</span>=<span className="attr-value">&quot;{tag.args[arg]}&quot;</span></span>);
		}

		lines.push(<div key={lines.length} className="code">
			{"\u{A0}".repeat(offset)}&lt;<span className="tag">{tag.name}</span> {args}{tag.children.length == 0 ? " /": null}&gt;
		</div>);
		tag.children.forEach(child => this.makeLine(lines, child, offset + 2));
		if (tag.children.length > 0){
			lines.push(<div key={lines.length} className="code">{"&nbsp;".repeat(offset)}&lt;/<span className="tag">{tag.name}</span>&gt;</div>);
		}
	}
}
