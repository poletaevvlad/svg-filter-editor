...
class Filter {
  constructor() {
    (this.primitives = []), (this.connections = []);
    this.primitivesById = {};
  ...
  removePrimitive(primitive) {
    ...
    this.connections = this.connections.filter((connection) => {
      if (
        connection.inputPrimitive == primitive.id ||
        connection.outputPrimitive == primitive.id
      ) {
  ...
  getOrderedPrimitives() {
    ...
    if (RESERVED_INPUT.indexOf(outputName) >= 0) {
      class PassthroughOutput extends Primitive {
        getSVG = () =>
          this.svgTag("feMerge").child(
            this.svgTag("feMergeNode").arg("in", outputName, null)
          );
      }
    ...
    let desiredResult = [
      `${outputPrimitiveConnection.outputPrimitive}-${outputPrimitiveConnection.outputIOID}`,
    ];
    let input = [];
    let result = [];
    var over = false;
    do {
      ...
      for (var i = 0; i < primitives.length; i++) {
        ...
        if (j == primitives[i].inputs.length) {
          primitives[i].outputs.forEach((o) =>
            input.push(`${primitives[i].id}-${o.id}`)
          );
