...
class Blend extends Primitive {
  ...
  getSVG() {
    return this.svgTag("feBlend")
      .arg("mode", this.mode, null)
      .input("in", 0)
      .input("in2", 1)
      .output("result", 0);
  }
...
class BlendNode extends Node {
  ...
  renderEditor() {
    return (
      <ComboBox
        value={this.props.primitive.mode}
        width={this.props.primitive.nodeWidth - 18}
        values={this.props.primitive.modes.map((mode) => {
          return { value: mode, label: mode };
        })}
        label="mode:"
        onChange={this.valSetter("mode")}
      />
    );
  }
}
