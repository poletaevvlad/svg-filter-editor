...
class ComponentTransferFunction{
	...
	fillSVGTagProps(tag, matrixMaker){
		...
		if (this.type == "table" || this.type == "discrete"){
			...
			tag.arg("amplitude", this.amplitude, null).arg("exponent", this.exponent, null)
...
class ComponentTransferFunctionComponent extends React.Component{
	render(){
		return <div className="vertical">
			<ComboBox value={this.props.function.type} width={this.props.width} label="type:" 
				values={this.props.function.types.map(val => {return {value: val, label: val}})} 
				onChange={val => this.props.onChange("type", val)}/>
			{this.props.function.type == "table" || this.props.function.type == "discrete" ? this.renderTableEditor() : null}
			{this.props.function.type == "linear" ? this.renderFieldEditor("slope", "slope") : null}
			{this.props.function.type == "linear" ? this.renderFieldEditor("intercept", "intercept") : null}
			{this.props.function.type == "gamma" ? this.renderFieldEditor("amplitude", "amplitude") : null}
			{this.props.function.type == "gamma" ? this.renderFieldEditor("exponent", "exponent") : null}
			{this.props.function.type == "gamma" ? this.renderFieldEditor("offset", "offset") : null}
		</div>
	}
	...
	renderFieldEditor(label, property){
		return <div className="horizontal">
			...
			<TextInput value={this.props.function[property]} onChange={val => this.props.onChange(property, val)} 
				validator={validators.isNumber} converter={converters.float}/>
		</div>
	}

	renderTableEditor(){
		return <TableEditor values={this.props.function.tableValues} onChange={(index, val) => {
				...
				if (after == this.props.function.tableValues.length - 1){
					...
					value = (this.props.function.tableValues[after] + this.props.function.tableValues[after + 1]) / 2;
				}
		...
		return <div className="graph">
			<svg width="140" height="100">
				<g transform="translate(1.5 3.5)">
					...
					{arrayRange(0, 10).map(i =>
						<line x1="3" y1={7 * i} x2="134" y2={7 * i} key={i} className="additional-line"/>
					)}
			...
			<div className="area">
				{arrayRange(0, this.props.values.length).map(i => 
					<div className="slider-container" key={i} style={{left: `${this.xForIndex(i)}px`}}>
						<ReactSlider max={100} value={this.props.values[i] * 100} orientation="vertical" 
							onChange={val => this.props.onChange(i, val / 100)} invert={true}/>
					</div>
				)}
			</div>
			{this.state.deletion ? 
				<div className="add-buttons">
					{this.props.type == "table" ? <div className="add-placeholder" /> : null}
					{this.props.values.length <= 2 ? null : arrayRange(0, this.props.values.length - (this.props.type == "table" ? 2 : 0)).map(i => 
						<div key={i} className="add-button" onClick={() => this.props.onRemoved(i + (this.props.type == "table" ? 1 : 0))}>-</div>
					)}
					{this.props.type == "table" ? <div className="add-placeholder" /> : null}
				</div> :
				<div className="add-buttons">
					{arrayRange(0, this.props.values.length - (this.props.type == "table" ? 1 : 0)).map(i => 
						<div key={i} className="add-button" onClick={() => this.props.onAdded(i)}>+</div>
					)}
				</div>
			}
		</div>
