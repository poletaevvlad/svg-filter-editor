...
class Image extends Primitive{
	constructor(){
		...
		this.aspectRatios = ["none", "xMinYMin", "xMidYMin", "xMaxYMin", "xMinYMid", 
			"xMidYMid", "xMaxYMid", "xMinYMax", "xMidYMax", "xMaxYMax"];
	}

	getSVG(){
		return this.svgTag("feImage").output("result", 0).arg("xlinkHref", this.url, null)
			.arg("x", this.x, null).arg("y", this.y, null).arg("width", this.width, null).arg("height", this.height, null)
...
class ImageNode extends Node{
	...
	renderEditor(){
		return <div className="vertical">
			...
			<TextInput value={this.props.primitive.url} onChange={this.valSetter("url")}/>
			...
			<div className="horizontal">
				...
				<TextInput className="field" value={this.props.primitive.width} onChange={this.valSetter("width")}  
					validator={validators.isNumber} converter={converters.float} />
				<div className="label inline-next">h:</div>
				<TextInput className="field" value={this.props.primitive.height} onChange={this.valSetter("height")}
					validator={validators.isNumber} converter={converters.float} />
			</div>
			
			<ComboBox value={this.props.primitive.preserveAspectRatio} width={this.props.primitive.nodeWidth - 18} 
				label="aspect ratio:" values={this.props.primitive.aspectRatios.map(val => {return {value: val, label: val}})} 
