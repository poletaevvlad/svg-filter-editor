...
class FilterEditor extends React.Component {
    ...
    return (
      <div id="app-root">
        <NodesContainer
          ...
          right={this.state.previewWidth}
        />
        <div
          id="result-preview"
          style={{ width: `${this.state.previewWidth}px` }}
        >
          <div id="separator" onMouseDown={this._separatorMouseDown}></div>
          <div id="toolbar">
            <div
              className={
                "section" + (this.state.editing == "code" ? " active" : "")
              }
              onClick={() => this._toggleEditor("code")}
            >
              Show code
            </div>
            <div
              className={
                "section" + (this.state.editing == "shape" ? " active" : "")
              }
              onClick={() => this._toggleEditor("shape")}
            >
              Shape
            </div>
            <div
              className={
                "section" +
                (this.state.editing == "background" ? " active" : "")
              }
              onClick={() => this._toggleEditor("background")}
            >
              Background
            </div>
            <div
              className="section short"
              onClick={() => this.setState({ aboutShown: true })}
            >
              ?
            </div>
          </div>
          ...
          {this.state.editing == "code" ? (
            ...
            <Preview
              ...
              bgCheckerboard={this.state.bgCheckerboard}
        ...
        {this.state.aboutShown ? (
          <AboutWindow onClose={() => this.setState({ aboutShown: false })} />
        ) : null}
      </div>
    );
  }
      ...
      return (
        <div id="preview-editor">
          <BackgroundEditor
            ...
            checkerboard={this.state.bgCheckerboard}
            onTypeChange={(val) => this.setState({ bgType: val })}
            onColorChanged={(val) => this.setState({ bgColor: val.hex })}
            onCheckerboardChanged={(val) =>
              this.setState({ bgCheckerboard: val })
            }
          />
        </div>
      );
      ...
      return (
        <div id="preview-editor">
          <ShapeEditor
            ...
            onWidthChange={(val) =>
              this.setState({ shapeWidth: parseFloat(val.replace(",", ".")) })
            }
            height={this.state.shapeHeight}
            onHeightChange={(val) =>
              this.setState({ shapeHeight: parseFloat(val.replace(",", ".")) })
            }
            strokeEnabled={this.state.strokeEnabled}
            onStrokeEnabledChange={(val) =>
              this.setState({ strokeEnabled: val.target.checked })
            }
            fillEnabled={this.state.fillEnabled}
            onFillEnabledChange={(val) =>
              this.setState({ fillEnabled: val.target.checked })
            }
            fillColor={this.state.fillColor}
            onFillColorChange={(val) => this.setState({ fillColor: val.hex })}
            strokeColor={this.state.strokeColor}
            onStrokeColorChange={(val) =>
              this.setState({ strokeColor: val.hex })
            }
            strokeWidth={this.state.strokeWidth}
            onStrokeWidthChange={(val) =>
              this.setState({ strokeWidth: parseFloat(val.replace(",", ".")) })
            }
            path={this.state.path}
            onPathChange={(event) =>
  ...
  _separatorMouseMove(e) {
    if (this._separatorDragging) {
      ...
      this.setState({
        previewWidth: Math.max(this.state.previewWidth - dx, MIN_PREVIEW_WIDTH),
      });
